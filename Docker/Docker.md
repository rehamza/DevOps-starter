...
it's only work linux https://docs.docker.com/network/drivers/host/

so for window we can create custom network

## Custom Network

docker network create currency-network

docker run  -d -p  8000:8000 --name=currency-exchange --network=currency-network hamzar111/currency-exchange:0.0.1.RELEASE


docker run  -d -p  8000:8000  --name=currency-conversion --env CURRENCY_EXCHANGE_URI=http://currency-exchange:8000 --network=currency-network hamzar111/currency-conversion:0.0.1.RELEASE
